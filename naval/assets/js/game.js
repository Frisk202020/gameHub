...
import { vwToPx, vhToPx, pxToVh, imgPathRoot, sfxPathRoot } from "./utilities.js";
...
function addDot(elem, color="white", ignoreMatrix=false){
    ...
    if (matrix[p.y - 1][p.x - 1] === "" || ignoreMatrix){
        ...
        if (!ignoreMatrix && state === "HOME"){
            if (color === "white"){
                new Audio(sfxPathRoot + "plouf.mp3").play();
            }
            else{
                new Audio(sfxPathRoot + "explosion.mp3").play();
            }
        }
    }
