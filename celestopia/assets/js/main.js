...
import { setDisableCaseHelper } from "./board/Case.js";
...
async function boardRenderLoop() {
    for (const p of players) {
        ...
        if (p.teleport) {
            ...
            p.movePawn().then(() => {
                if (p.ignoreTurnSystem) {
                    p.ignoreTurnSystem = false;
                }
                else {
                    nextPlayer(p);
                }
            ...
            const caseElm = board.elements[p.caseId];
            await p.caseResponse(caseElm.type);
            setDisableCaseHelper(false);
            if (caseElm.type === "ladder" || caseElm.type === "dice") {
                continue;
            }
            nextPlayer(p);
        }
...
async function nextPlayer(p) {
    ...
    p.disable();
    nextP.enable();
    if (nextP.boardId !== boardId) {
        changeBoard(nextP.boardId);
    }
}
async function main() {
    initPlayers();
