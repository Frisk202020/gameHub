...
export const debugTools = {
    ...
    diceNumber(id, n) { players[id - 1].diceNumber = n; },
    ...
    setPigAmmount(ammount) { pig.content = ammount; },
    addCoins(id, value) { players[id - 1].progressiveCoinChange(value); },
    addRibbons(id, value) { players[id - 1].progressiveRibbonChange(value); },
    addStars(id, value) { players[id - 1].progressiveStarChange(value); },
    setRich(id) {
        const p = players[id - 1];
    ...
    giveAquisition(id, name) {
        const aq = Aquisition.getByName(name);
        if (aq === undefined) {
            return "unrecognized aquisition";
        }
    ...
    giveWonder(id, name) {
        ...
        if (aq === undefined) {
            return "unrecognized wonder";
        }
        players[id - 1].addWonder(aq);
    },
    setBoardId(id, board) { players[id - 1].boardId = board; },
    setCaseId(id, n) {
        const p = players[id - 1];
        p.pendingCaseId = n;
        p.teleport = true;
