...
export class Pipe extends Item {
    constructor(p) {
        ...
        super(p, "Tuyau de téléportation", "Echangez de place avec le joueur de votre choix (l'effet de votre case de destination se sera pas appliqué).", "pipe", () => { new Event(tx, p); }, true);
        this.imgStyle.paddingBottom = "0px";
        rx.recv().then((victim) => {
            const pCase = p.caseId;
            const bId = p.boardId;
            p.ignoreTurnSystem = true;
            victim.ignoreTurnSystem = true;
...
class Event extends BoardEvent {
    constructor(tx, itemHolder) {
        ...
        super([BoardEvent.generateTextBox("Choisissez le joueur avec lequel vous allez échanger de place."), box], BoardEvent.unappendedOkSetup(), BoardEvent.denySetup(true, "Annuler", () => itemHolder.addItem(new Pipe(itemHolder))));
    }
}
