...
var _DataEvent_instances, _DataEvent_box, _DataEvent_appendFileBox;
...
export class FileEvent extends BoardEvent {
    constructor() {
        ...
        super(buttons, BoardEvent.unappendedOkSetup(), BoardEvent.denySetup(true, "Retour"));
    }
}
/* event classes */
class SaveEvent extends BoardEvent {
    constructor() {
        ...
        super([BoardEvent.generateTextBox("Sauvegarder"), form], BoardEvent.unappendedOkSetup(), BoardEvent.denySetup(true, "Retour", () => new FileEvent()));
    }
}
class SaveKey extends BoardEvent {
    constructor(filename) {
        ...
        super([p, form], BoardEvent.unappendedOkSetup(), BoardEvent.denySetup(true, "Retour", () => new FileEvent()));
    }
}
class LoadEvent extends BoardEvent {
    constructor() {
        ...
        super([BoardEvent.generateTextBox("Charger"), form], BoardEvent.okSetup(true, "Afficher les fichiers disponibles", () => new DataEvent()), BoardEvent.denySetup(true, "Retour", () => new FileEvent()));
    }
}
class DataEvent extends BoardEvent {
    constructor() {
        ...
        super([BoardEvent.generateTextBox("Fichiers disponibles"), box], BoardEvent.unappendedOkSetup(), BoardEvent.denySetup(true, "Retour", () => new LoadEvent()));
        _DataEvent_instances.add(this);
        _DataEvent_box.set(this, void 0);
        __classPrivateFieldSet(this, _DataEvent_box, box, "f");
        this.loadData();
    }
    async loadData() {
        const data = await sendDataRequest();
        if (data.length === 0) {
            __classPrivateFieldGet(this, _DataEvent_box, "f").appendChild(BoardEvent.generateTextBox("Aucun fichier n'a été trouvé..."));
            return;
        }
        data.forEach((x) => __classPrivateFieldGet(this, _DataEvent_instances, "m", _DataEvent_appendFileBox).call(this, x));
    }
}
_DataEvent_box = new WeakMap(), _DataEvent_instances = new WeakSet(), _DataEvent_appendFileBox = function _DataEvent_appendFileBox(name) {
    const box = document.createElement("div");
    ...
    __classPrivateFieldGet(this, _DataEvent_box, "f").appendChild(box);
};
...
class Message extends BoardEvent {
    constructor(messages) {
        removeOldMenu();
        super(messages.map((m) => BoardEvent.generateTextBox(m)), BoardEvent.okSetup(true), BoardEvent.denySetup(false));
    }
}
/* Request functions */
let registeredKey = "";
async function sendSaveRequest(name, key) {
    try {
        ...
        const res = await fetch("celestopia/save", {
            method: "POST",
        ...
        if (json.authentification !== "") {
            registeredKey = json.authentification;
        }
        return {
            success: true,
            authentification_error: false,
...
async function sendLoadRequest(name) {
    try {
...
function loadData(data) {
    ...
    if (players.length <= data.players.length) {
        for (let i = 0; i < players.length; i++) {
            ...
            const p = new Player(i + 1, pData.name, pData.icon, pData.color);
        ...
        for (let i = 0; i < data.players.length; i++) {
            const pData = data.players[i];
            const p = new Player(i + 1, pData.name, pData.icon, "red");
            p.loadData(pData, data.turnHolder === p.id);
            players[i] = p;
        }
...
async function loadHandler(event) {
    ...
    if (name !== null) {
        ...
        if (data === undefined) {
            new Message(["Echec du chargement..."]);
        }
        else {
            loadData(data);
            new Message(["Chargement effectué !"]);
...
async function saveHandler(event) {
    ...
    if (name !== null) {
        ...
        if (res.success) {
            ...
            if (res.key !== "") {
                messages.push(`Clé d'authentification: ${res.key}.`);
            }
...
async function authentification_handler(file, event) {
    event.preventDefault();
    const form = event.target;
    const data = new FormData(form);
...
function createSaveFileBox(data, filename) {
    const box = document.createElement("div");
    box.style.display = "flex";
    box.style.flexDirection = "column";
    box.style.overflowY = "scroll";
...
function createFileParagraph(textContent) {
